...
export function canSelectItem(basket, item) {
  ...
  if (status === 'POOLING') {
    return {
      canSelect: false,
      reason: 'En mutualisation, les lignes sont déjà sélectionnées et non modifiables',
    };
  }
  ...
  if (status === 'ORDERED' || status === 'CLOSED') {
    return {
      canSelect: false,
      reason: 'Les items sont verrouillés dans un panier commandé ou clôturé',
    };
  }
...
export function canPurgeItems(basket) {
  ...
  if (status === 'SENT') {
    return {
      canPurge: false,
      reason: 'En statut envoyé, les items désélectionnés restent visibles',
    };
  }
 ...
 */
export function canTransitionBasket(
  basket,
  targetStatus,
  itemSelectionState = {},
  allBaskets = []
